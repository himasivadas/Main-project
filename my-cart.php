...
header('location:login.php');
	...
	$value=array_combine($pdd,$quantity);
       ...
       mysqli_query($con,"insert into orders(userId,productId,quantity,total) values('".$_SESSION['id']."','$qty','$val34','".$_SESSION['tp']."')");
      $z=mysqli_insert_id($con);

      $sql2=mysqli_query($con,"select * from orders where `id`='$z'");
      $r=mysqli_fetch_array($sql2);
         $pid=$r['productId'];
			$src=mysqli_query($con,"select * from products where id='$pid'");
			$ro=mysqli_fetch_array($src);
			$Pqty=$ro['productPriceBeforeDiscount'];

			$Tq=($Pqty-$val34);
			$n=mysqli_query($con,"update products set productPriceBeforeDiscount='$Tq' where id='$pid'");

         header('location:payment.php?id=<?php echo $totalprice?>');
}
}
}
...
<div class="col-md-4 col-sm-12 cart-shopping-total">
	...
	<?php } else {
echo "Your shoppizng Cart is empty";
		}?>
</div>			</div>
		</div> 
		</form>
